...
const wchar_t *swcsstr(const wchar_t *haystack, const wchar_t *needle,
                       size_t length) {
...
BOOL CheckResourceStrings(LPVOID data, DWORD size, const char *first,
  ...
  second_pos =
      swcsstr(src, second, (size - strlen(first) - 1) / sizeof(wchar_t));
  if (second_pos == NULL) {
...
BOOL LoadFromMemory(char *filename) {
  ...
  addNumber = (addNumberProc)MemoryGetProcAddress(
      handle, reinterpret_cast<LPCSTR>(0xff));
  if (addNumber != NULL) {
  ...
  addNumber2 = (addNumberProc)MemoryGetProcAddress(
      handle, reinterpret_cast<LPCSTR>(0x01));
  ...
  if (resourceInfo != NULL) {
    ...
    _tprintf(_T("Memory resource data: %ld bytes at 0x%p\n"), resourceSize,
             resourceData);
    if (!CheckResourceStrings(resourceData, resourceSize,
  ...
  if (resourceInfo != NULL) {
    ...
    _tprintf(_T("Memory resource data: %ld bytes at 0x%p\n"), resourceSize,
             resourceData);
    if (!CheckResourceStrings(resourceData, resourceSize,
                              "This is ANSI string 1",
                              L"This is UNICODE string 1")) {
      result = FALSE;
    }
  } else {
    result = FALSE;
  ...
  if (resourceInfo != NULL) {
    ...
    _tprintf(_T("Memory resource data: %ld bytes at 0x%p\n"), resourceSize,
             resourceData);
    if (!CheckResourceStrings(resourceData, resourceSize,
                              "This is ANSI string 2",
                              L"This is UNICODE string 2")) {
      result = FALSE;
    }
  } else {
    result = FALSE;
  ...
  if (resourceInfo != NULL) {
    ...
    _tprintf(_T("Memory resource data: %ld bytes at 0x%p\n"), resourceSize,
             resourceData);
    if (!CheckResourceStrings(resourceData, resourceSize,
                              "This is ANSI string 3",
                              L"This is UNICODE string 3")) {
      result = FALSE;
    }
  } else {
    result = FALSE;
