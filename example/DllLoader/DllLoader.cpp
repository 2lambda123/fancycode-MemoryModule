...
void LoadFromFile(void) {
  ...
  resourceInfo =
      FindResource(handle, MAKEINTRESOURCE(VS_VERSION_INFO), RT_VERSION);
...
void LoadFromMemory(void) {
  ...
  resourceInfo =
      MemoryFindResource(handle, MAKEINTRESOURCE(VS_VERSION_INFO), RT_VERSION);
...
#ifdef _WIN64
  throwException =
      (throwExceptionProc)MemoryGetProcAddress(handle, "throwException");
  if (!throwException) {
...
LPVOID MemoryFailingAlloc(LPVOID address, SIZE_T size, DWORD allocationType,
...
LPVOID MemoryMockAlloc(LPVOID address, SIZE_T size, DWORD allocationType,
                       DWORD protect, void *userdata) {
  CallList *calls = (CallList *)userdata;
  CustomAllocFunc current_func =
      calls->alloc_calls[calls->current_alloc_call++];
  assert(current_func != NULL);
  return current_func(address, size, allocationType, protect, NULL);
}

BOOL MemoryMockFree(LPVOID lpAddress, SIZE_T dwSize, DWORD dwFreeType,
                    void *userdata) {
  CallList *calls = (CallList *)userdata;
...
void TestCleanupAfterFailingAllocation(void *data, size_t size) {
  ...
  InitAllocFuncs(&expected_calls, MemoryDefaultAlloc, MemoryDefaultAlloc,
                 MemoryDefaultAlloc, MemoryDefaultAlloc, MemoryFailingAlloc,
                 NULL);
...
LPVOID MemoryAllocHigh(LPVOID address, SIZE_T size, DWORD allocationType,
                       DWORD protect, void *userdata) {
