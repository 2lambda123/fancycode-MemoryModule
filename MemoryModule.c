...
static BOOL FinalizeSection(PMEMORYMODULE module,
  ...
  if (VirtualProtect(sectionData->address, sectionData->size, protect,
                     &oldProtect) == 0) {
...
static BOOL RegisterExceptionHandling(PMEMORYMODULE module) {
#ifdef _WIN64
  PRUNTIME_FUNCTION pEntry;
  DWORD count;
  PIMAGE_DATA_DIRECTORY pDir =
      GET_HEADER_DICTIONARY(module, IMAGE_DIRECTORY_ENTRY_EXCEPTION);
  if (pDir->Size == 0) {
    return TRUE;
  }
...
static BOOL UnregisterExceptionHandling(PMEMORYMODULE module) {
#ifdef _WIN64
  PRUNTIME_FUNCTION pEntry;
  PIMAGE_DATA_DIRECTORY pDir =
      GET_HEADER_DICTIONARY(module, IMAGE_DIRECTORY_ENTRY_EXCEPTION);
...
_MemorySearchResourceEntry(void *root, PIMAGE_RESOURCE_DIRECTORY resources,
                           LPCTSTR key) {
...
BOOL MemoryModuleTestsuite() {
  ...
  for (idx = 0; AlignValueDownTests[idx][0]; ++idx) {
    ...
    if (value != tests[2]) {
      printf("AlignValueDown failed for 0x%" PRIxPTR "/0x%" PRIxPTR
  ...
  for (idx = 0; AlignValueDownTests[idx][0]; ++idx) {
    ...
    if (value != tests[2]) {
      printf("AlignValueUp failed for 0x%" PRIxPTR "/0x%" PRIxPTR
             ": expected 0x%" PRIxPTR ", got 0x%" PRIxPTR "\n",
             tests[0], tests[1], tests[2], value);
      success = FALSE;
    }
